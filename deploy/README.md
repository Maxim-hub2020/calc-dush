...
Каталоги цен душевых и зеркал хранятся в PostgreSQL CRM и синхронизируются через защищённый `/api/calculator-settings/`. На каждом устройстве остаётся локальная копия для расчёта без сети. Для входа используется учётная запись администратора CRM.

Архив КП хранится в PostgreSQL CRM и синхронизируется через защищённый `/api/calculator-quotes/`. Локальная копия в `localStorage` остаётся для работы без сети и автоматически объединяется с серверным архивом после входа администратора.
