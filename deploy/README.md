...
Архив КП пока хранится в `localStorage`, поэтому архивы разных браузеров остаются раздельными. Для общего архива потребуется отдельная серверная модель КП.
